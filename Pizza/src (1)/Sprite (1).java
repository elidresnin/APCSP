...
public class Sprite {
	...
	public Sprite(int x_coordinate, int y_coordinate){
		...
		imageResource = new ImageResource("images/robot/", 1, 50);
		jumpCounter = -1;
	...
	public Rectangle getBounds(){
		if(x_direction < 0)
			return new Rectangle(x_coordinate + imageResource.getImageOffset()+45, y_coordinate+15, 
					imageResource.getImage().getIconWidth() - imageResource.getImageOffset()/2-40, 
					imageResource.getImage().getIconHeight()-15);
		else
			return new Rectangle(x_coordinate + 2 * imageResource.getImageOffset()+25, y_coordinate+15, 
					imageResource.getImage().getIconWidth() - imageResource.getImageOffset()/2-20, 
					imageResource.getImage().getIconHeight()-25);
	}
	...
	public void move(Component c){
		...
		// jump
				...
				(y_coordinate + imageResource.getImage().getIconWidth() < c.getHeight() && y_direction == 2 ))
			y_coordinate += (y_direction);

		if(jumpCounter >= 0 && jumpCounter < 160) {
			jumpCounter+=1;
			y_coordinate-=1;
		}
		else if(jumpCounter >= 160 && jumpCounter < 320){
			jumpCounter+=1;
			y_coordinate+=1;
		}
		else {
